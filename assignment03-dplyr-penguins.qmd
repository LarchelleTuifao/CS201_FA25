...
```{r, messages=FALSE}
unique(penguins_data$island) # an example of how to use unique()

fem_peng_mm <- penguins_data %>%
  filter(sex == "FEMALE", culmen_length_mm >= 39.0, island %in% c("Dream", "Torgersen"))

fem_peng_mm_larch <- penguins_data %>%
  filter(sex == "FEMALE", culmen_length_mm >= 39.0, island == "Dream" | island == "Torgersen")

View(fem_peng_mm)

unique(fem_peng_mm$island)
```
...
mutate_df <- penguins_data %>%
  mutate(flippometers = flipper_length_mm / 1e-6)
...
across_df <- penguins_data %>%
  group_by(island = "Biscoe", species) %>%
  summarize(across(is.numeric, max, na.rm = TRUE))
...
final_boss <- penguins_data %>%
  ...
  summarize(avg_mass_g = mean(body_mass_g, na.rm = TRUE))
