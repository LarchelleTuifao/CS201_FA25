...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  geom_histogram() +
  theme_bw() + 
  scale_fill_viridis_d(option="inferno") + # try adding option = "A", "C", "E", etc. These also have names! See documentation.
...
```{r, message=FALSE}
x_plot = ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species, shape = island)) +
          geom_point() +
          theme_bw() +
          scale_fill_viridis_d(option = "mako") +
          labs (title    = "Palmer Penguins, Distribution of Culmen Depth & Body Mass",
